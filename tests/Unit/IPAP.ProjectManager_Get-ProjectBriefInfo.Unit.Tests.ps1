...
Describe 'Get-ProjectBriefInfo Unit Tests' -Tag 'Get-ProjectBriefInfo', 'IPAP.ProjectManager' {
    ...
    Context '正常执行路径 - Normal Execution' {
        It '应返回两个值（格式化文本和项目名）' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return 'TestAuthor' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return 'OriginalTitle' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '中文标题' }
        ...
        It '返回值应包含项目名称' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return 'Author' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return 'MyProject' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '我的项目' }
            Mock -ModuleName IPAP.ProjectManager Read-MultiLineInput {
                param ($Prompt)
                if ($Prompt -like '*原文简介*')
                {
        ...
        It '返回值应包含作者中文译名' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return 'Author' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return 'Test' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '测试' }
            Mock -ModuleName IPAP.ProjectManager Read-MultiLineInput {
                param ($Prompt)
                if ($Prompt -like '*原文简介*')
                {
    ...
    Context '字符串处理测试' {
        It '原作品名应去除首尾空格' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return 'Author' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return '  TestProject  ' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '测试项目' }
            Mock -ModuleName IPAP.ProjectManager Read-MultiLineInput { return @() }
    ...
    Context '参数化调用测试' {
        ...
        It '参数为空字符串时应视为未提供，触发交互式输入' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return '交互式作者' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return '交互作品' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '中文标题' }
            Mock -ModuleName IPAP.ProjectManager Read-MultiLineInput {
                param ($Prompt)
                if ($Prompt -like '*原文简介*')
        ...
        It '参数为空白字符时应视为未提供，触发交互式输入' {
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作者名' } { return '作者' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '原作品名（原文）' } { return '标题' }
            Mock -ModuleName IPAP.ProjectManager Read-Host -ParameterFilter { $Prompt -eq '作品中文译名' } { return '中文标题' }
            Mock -ModuleName IPAP.ProjectManager Read-MultiLineInput {
                param ($Prompt)
                if ($Prompt -like '*原文简介*')
